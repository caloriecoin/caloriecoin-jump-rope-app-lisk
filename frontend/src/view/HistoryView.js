...
const HistoryView = ({navigation}) => {
    ...
    const handleUpdateMiningHistory = async () => {
        const userResponse = await axios.get(`https://caloriecoin.herokuapp.com/api/user/getUserAndWallet/${userInfo.id}`);
        if(userResponse)
        {
        ...
        const statsticsResponse = await axios.get('https://caloriecoin.herokuapp.com/api/minningJump/getOneUserMinningStatstics/' + userInfo.id);
        ...
        if(statsticsResponse)
        {
            ...
            if(statsticsData?.success)
            {
                ...
                setStatsticsXaxis(statsticsDataList);
            }
        }
    ...
    const handleUpdateBattleHistory = async () => {
        const response = await axios.get('https://caloriecoin.herokuapp.com/api/battleJump/getOneUserBattleJumps/' + userInfo.id);
    ...
    <View style={{flex:1, justifyContent:'center', alignItems:'center', backgroundColor:'white'}}>
        <View style={{flexDirection:'row', marginTop:12}}>
            <TouchableOpacity onPress={()=>{handleUpdateMiningHistory(); setType('mining')}}>
                <Text style={type === 'mining' ? styles.selectedText : styles.notSelectedText}>채굴기록</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>{handleUpdateBattleHistory(); setType('battle');}} style={{marginLeft:72}}>
                <Text style={type === 'battle' ? styles.selectedText : styles.notSelectedText}>배틀기록</Text>
            </TouchableOpacity>
        </View>
        ...
        {type === 'mining' &&
            <FlatList
                ListHeaderComponent={()=>{
                    return <View style={{width:'100%',justifyContent:'center', justifyContent:'center', marginTop:24,}}>
                        <Text style={{marginRight:'auto',marginLeft:4, marginBottom:10, fontFamily:'SUIT-SemiBold'}}>일자별 줄넘기 횟수</Text>
                        {graphData.length > 0 && <LineChart
                            data={{
                        ...
                        />}
                        <View style={{width:'100%',justifyContent:'center', justifyContent:'center',marginTop: 12, marginBottom:18}}>
                            <Text style={{textAlign:'center', fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>총 줄넘기 횟수</Text>
                            <Text style={{textAlign:'center', fontFamily:'SUIT-Heavy', fontSize: 28, color:'#000'}}>{totalJumps} <Text style={{fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>회</Text></Text>
                            <View style={{flexDirection:'row',justifyContent:'center', marginTop:24}}>
                                <View style={{justifyContent:'center', alignItems:'center'}} >
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>총 채굴 횟수</Text>
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-ExtraBold', fontSize: 18, color:'#000',marginTop:8}}>{totalEarnCalorieCoin} <Text style={{fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>CAL</Text></Text>
                                </View>
                                <View style={{justifyContent:'center', alignItems:'center', marginLeft:18}} >
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>총 칼로리 소모</Text>
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-ExtraBold', fontSize: 18, color:'#000',marginTop:8}}>{totalBurnedCalorie} <Text style={{fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>kcal</Text></Text>
                                </View>
                                <View style={{justifyContent:'center', alignItems:'center', marginLeft:18}} >
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-Regular',color:'#9ea4a9'}}>총 운동 시간</Text>
                                    <Text style={{textAlign:'center', fontFamily:'SUIT-ExtraBold', fontSize: 18, color:'#000',marginTop:8}}>{getSecondToString(totalDuration)}</Text>
                                </View>
                            </View>
                        </View>
                    </View>
