        ...
        if(type == 'mining')
        {
            ...
            response.data.map((item,index)=>{
                responseData.push({
                    ...
                    unit:'CAL',
                    subtitle:'',
                    description:`총 줄넘긴 횟수 : ${item.jumps_total}`
                });
            });
    ...
    <View style={{flex:1, justifyContent:'center', alignItems:'center', backgroundColor:'white'}}>
        <View style={{flexDirection:'row', marginTop:12}}>
            <TouchableOpacity onPress={()=>{callData('mining')}}>
                <Text style={type === 'mining' ? styles.selectedText : styles.notSelectedText}>채굴왕</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>{callData('battle')}} style={{marginLeft:72}}>
                <Text style={type === 'battle' ? styles.selectedText : styles.notSelectedText}>배틀왕</Text>
            </TouchableOpacity>
        </View>
        <FlatList
            data={data}
