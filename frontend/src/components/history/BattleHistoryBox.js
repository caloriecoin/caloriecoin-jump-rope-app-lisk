...
const BattleHistoryBox = ({history, item}) =>{
    ...
    return slector && <View style={styles.container}>
        <View style={styles.topContainer}>
            ...
            <Text style={slector?.battleResult == 0 ? styles.drawText : slector?.battleResult == 1 ? styles.winnerText: styles.loserText}>{slector?.battleResult == 0 ? '무' : slector?.battleResult == 1 ? '승' : '패'}</Text>
        </View>
